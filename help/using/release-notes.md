---
title: "[!DNL Adobe Experience Manager] notas de la versión de aplicación de escritorio"
...
# [!DNL Adobe Experience Manager] notas de la versión de aplicación de escritorio {#release-notes-v2}
...
El **compatible [!DNL Experience Manager] versiones** son:

* [!DNL Experience Manager] as a [!DNL Cloud Service]. Consulte [notas de la versión](https://experienceleague.adobe.com/en/docs/experience-manager-cloud-service/content/release-notes/home).
* [!DNL Experience Manager] 6.5.0 o posterior, en Adobe Managed Services (AMS) o local. Consulte [notas de la versión de service pack](https://experienceleague.adobe.com/es/docs/experience-manager-65/content/release-notes/release-notes).
...
El **descargar direcciones URL** para los sistemas operativos admitidos son:
...
Algunos tipos de archivo admiten la colocación de recursos vinculados en el archivo binario. La aplicación descarga previamente los recursos vinculados si el recurso está presente en el [!DNL Experience Manager] repositorio cuando estos archivos binarios se abren con la aplicación de escritorio. Los tipos de archivo admitidos actualmente son:

* [!DNL Adobe InDesign] archivos (formato INDD)
* [!DNL Adobe Illustrator] archivos (formato AI)
* [!DNL Adobe Photoshop] archivos (formato PS)

La función es compatible con [!DNL Adobe Creative Cloud] 2018 y [!DNL Adobe Creative Cloud] Versiones de 2019 de la aplicación anterior. La aplicación utiliza un método heurístico que busca la mejor coincidencia para asignar las rutas de escritorio locales de los recursos vinculados a las direcciones URL en la aplicación [!DNL Experience Manager] servidor. Se basa en algunos supuestos:

* Las rutas para colocar archivos en la aplicación nativa utilizan una ruta de escritorio global (desde el recurso compartido de red local que se muestra con la variable [!UICONTROL Reveal] opción).
...
* [!DNL Experience Manager] XMP ha extraído el registro de con las rutas al registro de metadatos del recurso.

* Las rutas pueden coincidir con los recursos en [!DNL Experience Manager], es decir, los archivos colocados también se encuentran en [!DNL Experience Manager] en una ruta coincidente.
...
* La aplicación de escritorio muestra un mensaje de error al ordenar los recursos haciendo clic en **[!UICONTROL Edited Locally]**.
...
* Las velocidades de carga y descarga de recursos han mejorado, incluso con recursos grandes. Esta versión ha corregido un problema que causaba que los recursos se cargaran con [!DNL desktop app] a veces se producía un error cuando se cargaban archivos muy grandes.

**Actualización en la aplicación 2.1.2.0**

* Una nueva opción para [!UICONTROL Clear Cookies] se añade al menú principal de la aplicación. Ayuda con posibles problemas de inicio de sesión, por ejemplo al cambiar una conexión de un servidor a otro. Consulte [borrar cookies antes de conectarse](/help/using/troubleshoot.md#cannot-login-cookies-issue).

* Se ha añadido una nueva opción que, si se selecciona, permite a la aplicación cargar carpetas y archivos con nombres de nodo en [!DNL Adobe Experience Manager] coincide con los nombres de archivo y carpeta local. Este proceso garantiza la coherencia entre los nombres locales y cargados.

  Este comportamiento es similar al comportamiento predeterminado en la versión 1 de la aplicación de escritorio. Mientras que en la versión actual, si la opción no está activada, se muestran los espacios en blanco y los caracteres `% ; # , + ? ^ { } "` en los nombres de carpeta se reemplazan con guiones en las rutas de carpeta. Además, los caracteres en mayúsculas se convierten en minúsculas en las rutas de carpetas. Sin embargo, en los nombres de archivo, los caracteres `# % { } ? &` se sustituyen por guión, pero se conservan los espacios en blanco y la carcasa. Para obtener más información, consulte [Preferencias de aplicación](/help/using/install-upgrade.md#set-preferences) y [Carga y adición de nuevos recursos](/help/using/using.md#upload-and-add-new-assets-to-aem).

**Actualización en la aplicación 2.1.1.0**

* Una configuración avanzada permite que la aplicación emule el comportamiento de la aplicación v1.10 al cargar carpetas. En la versión 1.10, los nombres de nodo creados en el repositorio respetan los espacios y mayúsculas de los nombres de carpeta proporcionados por el usuario. En la versión 2.1, el comportamiento predeterminado no cambia: los espacios múltiples en los nombres de carpeta se sustituyen por guiones en el nombre del nodo del repositorio y los nombres de nodo se convierten a minúsculas. Consulte [las preferencias de aplicación](/help/using/install-upgrade.md#set-preferences).

**Actualización en la aplicación 2.1.0.0**

* Para cargar recursos, los usuarios ahora pueden arrastrar los archivos o carpetas a la interfaz de la aplicación, directamente desde el Explorador de Windows o el Buscador de Mac. Este proceso funciona además de la opción de carga disponible en la aplicación. Consulte [cargar recursos](/help/using/using.md#upload-and-add-new-assets-to-aem) <!-- CQ-4309527 -->

**Actualización en la aplicación 2.0.3**

El error corregido en esta versión es:

* Se ha corregido el problema de inicio de sesión para los usuarios de la aplicación en Windows que intentan acceder al repositorio DAM en [!DNL Adobe Experience Manager] 6.5.5.0
...
* Opción Permitir para configurar `%Temp%` directorio para coincidir `%APPDATA%` ruta. <!-- CQ-4282665 -->

* Permitir que los usuarios inicien sesión [!DNL Experience Manager] Autor mediante autenticación SAML de Okta. <!-- CQ-4278134 -->

## Instrucciones de instalación {#installation-instructions-v2}

Para saber cómo instalar y configurar la aplicación, consulte [Instalar [!DNL Experience Manager] aplicación de escritorio](install-upgrade.md).

Si está actualizando desde una versión anterior [!DNL Experience Manager] aplicación de escritorio, debe seguir estas prácticas recomendadas para la transición que se enumeran en [actualizar desde la versión anterior](install-upgrade.md#upgrade-from-previous-version).
...
* La aplicación proporciona control total sobre las operaciones que requieren la transferencia completa de archivos binarios de recursos desde y hacia [!DNL Experience Manager] (**Abrir**, **Editar**, **Cargar cambios**, y **Cargar Assets**).
   ...
   * La aplicación no es un &quot;cliente de sincronización&quot; que sincroniza recursos en el escritorio y [!DNL Experience Manager].

   * La aplicación no proporciona un recurso compartido de red que asigne la variable [!DNL Experience Manager] repositorio como estructura de carpetas virtuales.
...
![Flujo de recursos desde [!DNL Experience Manager] del servidor a las aplicaciones de escritorio nativas a través de la aplicación de escritorio](assets/da20_flow_diagram.png)

## Problemas conocidos {#known-issues-v2}

**Problemas de la interfaz de usuario:**

* A veces, la interfaz de la aplicación de escritorio puede quedar en blanco. Haga clic con el botón derecho y seleccione [!UICONTROL Refresh] para volver a cargar la aplicación. Después de esta actualización, debe comenzar en la raíz del repositorio de DAM. Se conservan las actualizaciones o los estados de los recursos. <!-- CQ-4270267 -->
...
* En ocasiones, cuando se conecta a una dirección URL que no tiene un [!DNL Experience Manager] servidor en ejecución, la pantalla de conexión deja de responder. Salga de la aplicación y vuelva a iniciarla.

**Problemas de CRUD (Crear, Leer, Actualizar y Eliminar):**

* Al cargar cambios en un recurso con comentarios, los comentarios se almacenan con el recurso en [!DNL Experience Manager] pero no son visibles como comentarios de versiones. Este problema se resuelve en [!DNL Experience Manager] 6.4.5 y [!DNL Experience Manager] 6.5.1. El Adobe recomienda instalar los Service Pack más recientes. <!-- CQ-4268990 -->
...
>[!MORELIKETHIS]
>
>* [[!DNL Experience Manager] as a [!DNL Cloud Service] documentación](https://experienceleague.adobe.com/es/docs/experience-manager-cloud-service)
>* [[!DNL Experience Manager] as a [!DNL Cloud Service] [!DNL Assets] documentación](https://experienceleague.adobe.com/es/docs/experience-manager-cloud-service/content/assets/overview)
