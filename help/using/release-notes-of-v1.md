...
description: Detalles de la versión, mejoras, nuevas funciones, compatibilidad y vínculos de descarga para la versión 1.10 de la aplicación de escritorio de AEM.
...
Adobe recomienda utilizar la última versión de la aplicación de escritorio de AEM para asegurarse de que está utilizando las últimas funciones, las correcciones de errores más recientes y el mejor rendimiento posible.

La versión de la aplicación de escritorio de AEM que planea instalar en su equipo local requiere un servidor de AEM específico
versión / componentes adicionales del lado del servidor (paquetes de servicio, correcciones rápidas o paquetes de funciones). Asegúrese de que el servidor de AEM esté configurado correctamente antes de conectarse a él por primera vez. Si necesita ayuda, póngase en contacto con el administrador de AEM.
...
AEM desktop app 1.10 se centra en mejorar la experiencia del usuario en cargas grandes, información sobre las operaciones en segundo plano y experiencia optimizada al abrir recursos con archivos vinculados (como InDesign).

>[!NOTE]
>
>Si usa macOS 10.15.4 o posterior, use al menos la versión 1.10.0.6 de la aplicación. Esta versión del parche cumple con [los requisitos de certificación de Apple](https://developer.apple.com/news/?id=04102019a).
...
Para obtener instrucciones detalladas, consulte [Instalar y configurar la aplicación de escritorio de AEM](install-configure-app-v1.md).
...
* **Versión 1.9 / 1.9.1**: cargas reanudables, ventana de estado mejorada, iconos de aplicación que indican el estado de la aplicación/conexión, recuperación previa de recursos vinculados para archivos InDesign.
...
La aplicación de escritorio de AEM 1.9 se centró en mejorar la experiencia del usuario en relación con las cargas grandes, la información sobre las operaciones en segundo plano y la experiencia optimizada al abrir recursos con archivos vinculados (como InDesign).
...
* Al descargar documentos de InDesign con recursos vinculados de AEM Assets, la aplicación de escritorio descarga primero todos los recursos vinculados antes de abrir el documento e indicar el estado de descarga. Por ejemplo, 5 de 24.

[!UICONTROL Bulk Uploads]

La carga de jerarquías de carpetas grandes a través de [!UICONTROL Create] > [!UICONTROL Upload Folder] en la interfaz de usuario web de AEM déclencheur este cuadro de diálogo. Lo mismo ocurre cuando copia y selecciona &quot;Pegar Assets&quot; en Finder o Explorer en el menú contextual de la aplicación de escritorio.
...
El icono de la aplicación indica el estado de las operaciones en segundo plano para proporcionar una mejor pista visual a los usuarios. Por ejemplo, cuando la aplicación no está conectada a AEM, el icono aparece atenuado. Cuando hay una carga activa, se muestra una superposición de &quot;sincronización&quot;, etc.

**Recuperación previa de Assets enlazado**

Para mejorar la experiencia del usuario con los documentos de InDesign que contienen recursos vinculados almacenados en AEM, la aplicación de escritorio recupera previamente estos archivos vinculados en la caché local. Este flujo se produce antes de que descargue y abra el documento de InDesign. De este modo, el usuario tiene los archivos vinculados disponibles localmente y no necesita esperar más al acceder a los recursos en InDesign (en el panel Vínculos).
La recuperación previa solo funciona si AEM reconoce los vínculos en el servidor. Un recurso con vínculos reconocidos tiene una lista de &quot;Referencias&quot; enumerada en la vista Propiedades del recurso de InDesign.

### Mejoras disponibles desde la aplicación de escritorio de AEM 1.8.x {#enhancements-available-since-aem-desktop-app-18x}

La versión de seguimiento rápido 1.8.1 de la aplicación de escritorio de AEM añadió mejoras al abrir varios archivos a la vez desde la interfaz de usuario de AEM a la versión 1.8 (CQ-4237747, CQ-4238780). Las mejoras de la aplicación de escritorio 1.8 de AEM son las siguientes:
...
* Inicio de sesión: (Win) inicio de sesión fijo en una instancia de AEM configurada para utilizar SAML y SSL (CQ-4216353)
...
* Red:
   ...
   * (Mac) Agregue compatibilidad para conectarse a AEM mediante la configuración del proxy `.pac` (CQ-4233430).
...
* Estabilidad:
   ...
   * Se ha mejorado la estabilidad cuando la aplicación de escritorio de AEM se conecta a un servidor de AEM sobrecargado (CQ-4224803).
...
* Instalación y desinstalación:
   ...
   * El instalador de [Windows de 32 bits] no puede instalar Microsoft .NET Framework v. 4.5 (CQ-4218084).

   * (Mac) Script manual para eliminar por completo los archivos de la aplicación de escritorio (CQ-4216489).

>[!NOTE]
>
>Los problemas encontrados en las cargas beta de la aplicación de escritorio de AEM 1.7 que estaban ausentes en la versión 1.6 se omiten de las notas de la versión.
...
* Proceso de inicio de sesión mejorado en AEM:
...
* Problemas seleccionados encontrados y corregidos en la beta 1.6:
   ...
   * Error al arrastrar y soltar archivos que generan un error al cargar en AEM (CQ-4217006).
...
* Nuevas funciones y mejoras: Añada la funcionalidad Copiar/Pegar a la integración de Finder para permitir la transferencia directa desde el escritorio a AEM (CQ-4208158).

* Correcciones de errores:

   * Se ha corregido el error 43 que se producía a veces durante el cambio de nombre de recursos (CQ-4207900).

   * La reversión a una versión anterior desde la cronología en AEM no actualiza el recurso en el Buscador (CQ-4205194).
   ...
   * Al colocar un archivo en InDesign por primera vez se inicia una advertencia de actualización (CQ-4207454).

Nota sobre las advertencias de vínculos: las aplicaciones de Creative Cloud (como InDesign) toman una &quot;instantánea&quot; de la hora de la última modificación del elemento en el momento en que se coloca. Si esa fecha cambia en un momento posterior, la aplicación de Adobe Creative Cloud informa de que los vínculos no están actualizados. Se informa de esta información de un par de maneras:
...
Este comportamiento es el mismo para los recursos del disco local y los recursos de un directorio montado en el escritorio de AEM, con las siguientes excepciones:
...
* Si un usuario modifica un recurso colocado a través del directorio montado del escritorio de AEM y luego borra su caché local, el recurso colocado se registra como desactualizado.

Ambos casos son previsibles y son efectos secundarios de la arquitectura de &quot;sincronización retrasada&quot; de los equipos de escritorio de AEM.

**Versión 1.5.0.x para macOS X y Windows:** Esta versión de la aplicación de escritorio de AEM ofrece las siguientes ventajas:

* Mejor estabilidad y resiliencia frente a los problemas de red.

   * Asignación más estable de AEM Assets (CQ-103276, CQ-4204669, CQ-4203957).
...
* Optimizaciones en la comunicación de red con el servidor de AEM (CQ-4204974, CQ-100903).

* Mayor fiabilidad al abrir, colocar y guardar recursos de AEM en aplicaciones de Creative Cloud (CQ-4203968, CQ-4205511, CQ-103543, CQ-4207141, CQ-90980).
...
* Carga simplificada de carpetas jerárquicas mediante la nueva acción Crear > Cargar carpeta en la IU táctil.
   ...
   * La aplicación de escritorio atraviesa la jerarquía de carpetas determinada en el escritorio en segundo plano y carga los archivos a los AEM Assets.
...
* Los archivos se guardan localmente primero y luego se vuelven a cargar en AEM en segundo plano después de un tiempo predefinido (30 segundos). Este flujo de trabajo reduce el tiempo para guardar archivos.
* Mejor gestión de las aplicaciones de escritorio que utilizan operaciones de archivo intermedias para guardar un archivo (guardados parciales y archivos temporales), lo que permite que la cronología de los recursos de AEM muestre la versión correcta y la información de carga de recursos.
...
El punto de montaje de la aplicación de escritorio de AEM ha cambiado en las versiones 1.4 y 1.5. En macOS, se cambió a una subcarpeta DAM dentro de la carpeta local del usuario, que admite usuarios no administradores (CQ-104183).

Debido a que la carpeta `/Volumes` ya no requiere derechos administrativos, este cambio se revirtió en 1.5.1. Este cambio también permite compartir documentos de InDesign que han colocado recursos de AEM entre usuarios de macOS.
...
* macOS X:
   ...
   * El repositorio de AEM montado con la aplicación de escritorio se ve como una unidad de red `smb` en el Finder, en lugar de como una unidad WebDAV.
* Windows:
   * El protocolo de unidad de red local para las integraciones de escritorio de Windows permanece; AEM se monta como recurso compartido WebDAV.
* Para ambas plataformas (Windows y Mac):
   * El protocolo para acceder/descargar recursos y cargar cambios en AEM se ha cambiado al protocolo nativo de AEM, que es un protocolo RESTful basado en HTTP. Proporciona un mayor control sobre las operaciones de red y es más compatible con la infraestructura de red.
...
* En la primera fase, se guarda un archivo localmente. De este modo, el usuario que guarde el archivo no tendrá que esperar hasta que el archivo se transfiera completamente a AEM y podrá reanudar el trabajo en cuanto se guarde el archivo.
* En la segunda fase, la aplicación de escritorio carga un archivo actualizado en el servidor de AEM después de un retraso predefinido (por ejemplo, treinta segundos). Esta operación se produce en segundo plano. Use la opción **Mostrar estado de sincronización de archivos en segundo plano** para ver el estado de la operación de carga.

## Avisos importantes {#important-notices}

**Carga de carpeta.** Adobe recomienda utilizar la nueva capacidad de carga de carpetas para cargar carpetas jerárquicas de mayor tamaño en AEM. Se recomienda este método en lugar de utilizar una copia o arrastrar y soltar en un repositorio de AEM montado desde el nivel de Finder/Explorer. Al utilizar la función de carga de carpetas, la aplicación de escritorio se comunica directamente con AEM y, por lo tanto, tiene un control mucho mejor sobre el proceso general.

**Mantener la sesión de AEM disponible.** La aplicación de escritorio de AEM depende de una sesión abierta en el servidor de AEM Assets para garantizar un funcionamiento adecuado. Los usuarios diarios deben desmontar los AEM Assets al final del día para cerrar la sesión y volver a montarlos por la mañana para garantizar el inicio de sesión y la funcionalidad de uso compartido de red.

**Desactivar &quot;Vista previa de iconos&quot; en el Finder.** Para una exploración eficaz de las carpetas grandes con Finder, especialmente con una conectividad de red deficiente, asegúrese de que tanto &quot;Icon&quot; como &quot;Icon Preview&quot; estén desactivados. De lo contrario, Finder comienza a descargar cada recurso en una carpeta para generar una pequeña previsualización, lo que puede provocar un rendimiento deficiente y un uso elevado del ancho de banda (CQ-4219779)

* En Finder, vaya a la carpeta de red compartida AEM Assets
...
**Limpie la caché al conectarse a un nuevo servidor de AEM.** Si la aplicación de escritorio se conecta a otro servidor de AEM con la misma dirección URL, la caché no se borra automáticamente. Borre la caché manualmente para garantizar las operaciones adecuadas. Tenga en cuenta que este proceso generalmente se produciría en pruebas, cuando las instalaciones de AEM se pueden reemplazar mientras se ejecutan en la misma dirección URL (CQ-4216982)

**Usar certificados SSL firmados por la CA.** La aplicación de escritorio de AEM no admite certificados SSL autofirmados al conectarse a AEM mediante una conexión HTTPS segura. Se requiere un certificado firmado por la CA en el servidor para estas conexiones. (CQ-87941)
...
* Nombres de archivo / localización:
   * Los nombres de archivo y carpeta con caracteres reservados no se gestionan correctamente. Asegúrese de utilizar nombres de archivo y carpeta que se ajusten a los requisitos de AEM. (CQ-93361, CQ-93308, CQ-89276, CQ-4217183)
   * Algunas aplicaciones, como Adobe Illustrator, pueden crear archivos con nombres que no se admiten en AEM. Por ejemplo, agregar `Converted` después de convertir un archivo, lo que impide que se cargue. (CQ-4216985)
...
* Integración de Finder:
   ...
   * Open in AEM Assets... se limita a selecciones de carpeta y un solo archivo. (CQ-83382)
...
La aplicación de escritorio de AEM funciona con varias versiones de AEM. Consulte la matriz de compatibilidad para ver las versiones compatibles.

| Versión | Revisión | Fecha de lanzamiento | Compatibilidad |
|--- |--- |--- |--- |
| 1,10 | 1.10.0.3 (Mac y Win) | sábado, 31 de agosto de 2018 | AEM 6.5; AEM 6.4 SP1; AEM 6.3 SP2; AEM 6.2 SP1 CFP2+; AEM 6.1 SP2 CFP7+ |
| 1,9 | 1.9.1.1 (Mac y Win) | 21 de junio de 2018 | AEM 6.4; AEM 6.3 SP1; AEM 6.2 SP1 CFP2+; AEM 6.1 SP2 CFP7+ |
| 1.8 | 1.8.1.0 (Mac y Win) | jueves, 28 de marzo de 2018 | AEM 6.4; AEM 6.3 SP1; AEM 6.2 SP1 CFP2+; AEM 6.1 SP2 CFP7+ |
| 1,7 | 1.7.0.3 (Mac y Win) | jueves, 10 de enero de 2018 | AEM 6.3 SP1; AEM 6.2 SP1 CFP2+; AEM 6.1 SP2 CFP7+ |
